document.addEventListener('DOMContentLoaded', () => {
    ...
    const voiceBtn = document.getElementById('iniyaVoiceBtn');
    ...
    if (SpeechRecognition) {
        ...
        recognition.onresult = (event) => {
            ...
            const wakeMatch = /\b(open iniya|open iniya ai|open chatbot|open assistant|hey iniya|hi iniya|hello iniya|ok iniya|iniya|iniya ai|திற இனியா|இனியாவை திற|ஹே இனியா|இனியா)\b/i.test(lowerTranscript);

            if (wakeMatch) {
                if (!modal.classList.contains('active')) {
                    openChat();
                    const remainingQuery = lowerTranscript.replace(/\b(open iniya|open iniya ai|open chatbot|open assistant|hey iniya|hi iniya|hello iniya|ok iniya|iniya|iniya ai|திற இனியா|இனியாவை திற|ஹே இனியா|இனியா)\b/gi, '').trim();
                    if (remainingQuery.length > 2) {
                        handleUserQuery(remainingQuery);
                    } else {
                        addBotMessage(currentLang === 'ta' ? "வணக்கம்! நான் தயார், சொல்லுங்க!" : "Hi! I'm awake and ready. How can I help you today?");
                    }
                } else if (isManualListening) {
        ...
        ['click', 'mousemove', 'keydown', 'touchstart'].forEach(evt => {
            document.addEventListener(evt, startListening, { once: true });
        });
